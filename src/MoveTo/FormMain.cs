...
namespace MoveTo
{
    public partial class FormMain : Form
    {
        ...
        private void SetupForm()
        {
            ...
            // 数字8文字+ _ 形式の場合はその形式のフォルダ名に変更
            if (Regex.IsMatch(this.targets.LatestTargetName, @"^\d{8}_"))
            {
                header = this.targets.LatestTargetName.Substring(0, 9);
                name = this.targets.LatestTargetName.Substring(9);
            }
